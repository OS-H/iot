...
class TBDeviceMqttClient:
    def __init__(self, host, port=1883, token=None, quality_of_service=None):
        ...
        self.__port = port
        if token == "":
            self._client = paho.Client(client_id=''.join(choice(ascii_lowercase) for _ in range(20)))
            log.warning("token is not set, connection without tls wont be established")
        else:
            self._client = paho.Client(client_id=token)
    ...
    def _on_publish(self, client, userdata, result):
        # log.debug("Data published to IoT!")
        pass

    def _on_disconnect(self, client, userdata, result_code):
        self.__is_connected = False
        log.info("Disconnected client: %s, user data: %s, result code: %s", str(client), str(userdata), str(result_code))
    ...
    def disconnect(self):
        try:
            ...
            log.exception(e)
        log.debug("Disconnecting from IoT")
        self.__is_connected = False
    ...
    def publish_data(self, data, topic, qos):
        ...
        log.debug("Publish topic=%s data=%s", topic, data)
                ...
                if not self.__timeout_queue.empty():
                    ...
                    if item is not None:
                        ...
                        if callback is not None:
                            callback(None, TBTimeoutException("Timeout while waiting for a reply from IoT!"))
                else:
                    time.sleep(0.01)
            except Exception as e:
                log.exception(e)
