...
class TBGatewayMqttClient(TBDeviceMqttClient):
    ...
    def _on_connect(self, client, userdata, flags, result_code, *extra_params):
        ...
        try:
            if result_code == 0:
                sub1=self._client.subscribe(GATEWAY_ATTRIBUTES_TOPIC, qos=1)[1]
                if sub1 is not None:
                    self._gw_subscriptions[int(sub1)] = GATEWAY_ATTRIBUTES_TOPIC
                    ...
                    log.error("gateway subscribe failed.")
    ...
    def gw_connect_device(self, device_name, device_type):
        ...
        log.info("Connected device %s", device_name)
        return info
    ...
    def gw_disconnect_device(self, device_name):
        ...
        log.info("Disconnected device %s", device_name)
        return info
