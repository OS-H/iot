...
class MqttConnector(Connector, Thread):
    def __init__(self, gateway, config, connector_type):
        ...
        # RPCs, i.e., remote procedure calls (IoT towards devices) handlers
        self.load_handlers('serverSideRpc', mandatory_keys['serverSideRpc'], self.__server_side_rpc)

        # Connect requests, i.e., telling IoT that a device is online even if it does not post telemetry
        self.load_handlers('connectRequests', mandatory_keys['connectRequests'], self.__connect_requests)

        # Disconnect requests, i.e., telling IoT that a device is offline even if keep-alive has not expired yet
        self.load_handlers('disconnectRequests', mandatory_keys['disconnectRequests'], self.__disconnect_requests)

        # Shared attributes direct requests, i.e., asking IoT for some shared attribute value
        self.load_handlers('attributeRequests', mandatory_keys['attributeRequests'], self.__attribute_requests)

        # Attributes updates requests, i.e., asking IoT to send updates about an attribute
    ...
    def _on_message(self, client, userdata, message):
        ...
        if topic_handlers:
            ...
            # may produce more than one message towards IoT. This also means that I cannot return after
                        ...
                        if converted_content:
                            ...
                            if isinstance(converted_content,list):
                                for item in converted_content:
                                    self.__gateway.send_to_storage(self.name, item)
        ...
        if topic_handlers:
            for topic in topic_handlers:
                ...
                # Note: device must be added even if it is already known locally: else IoT
