...
class RemoteConfigurator:
    ...
    def process_configuration(self, configuration):
        try:
            if not self.in_process:
                ...
                self.__new_general_configuration_file = self.__new_configuration.get("IoT")
    ...
    def send_current_configuration(self):
        try:
            ...
            current_configuration["IoT"] = self.__old_general_configuration_file
            current_configuration["IoT"]["logs"] = b64encode(self.__old_logs_configuration.replace('\n', '}}').encode("UTF-8"))
            ...
            LOG.debug('Current configuration has been sent to IoT: %s', json_current_configuration)
        except Exception as e:
            LOG.exception(e)
    ...
    def __process_connectors_configuration(self):
        ...
        if self.__safe_apply_connection_configuration():
            ...
            with open(self.__gateway.get_config_path() + "gateway.yaml", "w", encoding="UTF-8") as general_configuration_file:
    ...
    def __prepare_connectors_configuration(self, input_connector_config):
        try:
            ...
            for connector in input_connector_config['IoT']['connectors']:
    ...
    def __safe_apply_connection_configuration(self):
        ...
        try:
            ...
            self.__gateway.tb_client = TBClient(self.__new_general_configuration_file["IoT"])
            ...
            if not connection_state:
                ...
                LOG.info("The gateway cannot connect to the IoT server with a new configuration.")
                return False
            else:
                self.__old_tb_client.stop()
    ...
    def __revert_configuration(self):
        try:
            ...
            self.__gateway.tb_client = TBClient(self.__old_general_configuration_file["IoT"])
            self.__gateway.tb_client.connect()
            self.__gateway.subscribe_to_required_topics()
