...
from gateway.tb_gateway_remote_configurator import RemoteConfigurator
from gateway.tb_remote_shell import RemoteShell
from gateway.tb_gateway_update_configurator import UpdateConfigurator
...
class TBGatewayService:
    def __init__(self, config_file=None):
        ...
        if config_file is None:
            config_file = path.dirname(path.dirname(path.abspath(__file__))) + '/config/gateway.yaml'.replace('/', path.sep)
        ...
        self.tb_client = TBClient(config["isthings"])
        self.tb_client.connect(60)
        ...
        if config["isthings"].get("remoteShell"):
            log.warning("Remote shell is enabled. Please be carefully with this feature.")
            self.__remote_shell = RemoteShell(platform=system, release=release())
        ...
        if config["isthings"].get("remoteConfiguration"):
            try:
        ...
        self.__update_configurator = None
        self.__update_last_time = 0
        self.__update_period = config["isthings"].get("autoLoadConfigurationPeriod")
        if self.__update_period:
            try:
                self.__update_configurator = UpdateConfigurator(self, config)
            except Exception as e:
                log.exception(e)
        self.__load_persistent_devices()
        self._published_events = Queue(-1)
        self._send_thread = Thread(target=self.__read_data_from_storage, daemon=True,
                                   name="Send data to IoT Thread")
        self._send_thread.start()
        if config["isthings"].get("deviceConnectPeriod"):
            period=config["isthings"].get("deviceConnectPeriod")
            if period > 0:
                self._device_connect_thread=Thread(target=self.__publish_connected_devices,daemon=True,args=(period,))
                self._device_connect_thread.start()
        log.info("Gateway started.")
        sleep(3)

        try:
            gateway_statistic_send = 0
            check_period = 10000.0
            while not self.stopped:
                ...
                if self.__update_configurator and cur_time - self.__update_last_time > self.__update_period*1000:
                    self.__update_configurator.process_config(config)
                    self.__update_last_time = time()*1000
                
                if cur_time - gateway_statistic_send > check_period:
                    if self.tb_client.is_connected():
                        ...
                        check_period = 60000.0
                        count = self._event_storage.get_event_count()
                        log.info("eventsBuffer = %s",count)
            ...
            if content is not None:
                ...
                if new_configuration is not None and self.__remote_configurator is not None:
                    try:
                        ...
                        # if confirmed:
                            # self._send_thread = Thread(target=self.__read_data_from_storage, daemon=True,
                            #                            name="Send data to isthings Thread")
                            # self._send_thread.start()
                        self.__remote_configurator.send_current_configuration()
                    except Exception as e:
                        log.exception(e)
        ...
        if not connector_name == self.name:
            ...
            if data["deviceName"] not in self.get_devices():
                self.add_device(data["deviceName"],
                                {"connector": self.available_connectors[connector_name]}, wait_for_publish=False, device_type=data["deviceType"])
            if connector_name not in self.__connector_incoming_messages:
                self.__connector_incoming_messages[connector_name] = 0
            else:
                self.__connector_incoming_messages[connector_name] += 1
        else:
            data["deviceName"] = "currentIsthingsGateway"
        ...
        try:
            for device in devices_data_in_event_pack:
                if devices_data_in_event_pack[device].get("attributes"):
                    if device == self.name or device == "currentIsthingsGateway":
        ...
        try:
            ...
            if device is None:
                if content.get("params") is not None:
                    device = content["params"].get("deviceTo")
                    if device is not None:
                        content={
                            "device":device,
                            "data":content
                            }
                        content["data"]["id"]=request_id
                else:
                    device = None
            if device is not None:
    ...
    def __form_statistics(self):
        summary_messages = {"eventsProduced": 0, "eventsSent": 0, "eventsBuffer": 0}
        telemetry = {}
        ...
        summary_messages["eventsBuffer"] = self._event_storage.get_event_count()
        return summary_messages

    def add_device(self, device_name, content, wait_for_publish=False, device_type=None):
        log.debug("add device %s",device_name)
    ...
    def __publish_connected_devices(self,period):
        ts_old = time() - period
        while not self.stopped:
            try:
                if time() - ts_old > period and self.tb_client.is_connected():
                    for device in self.__connected_devices:
                        self.tb_client.client.gw_connect_device(device, 
                            self.__connected_devices[device].get('device_type', 'default'))
                        sleep(.1)
                    ts_old=time()
                sleep(1)
            except Exception as e:
...
if __name__ == '__main__':
    TBGatewayService(path.dirname(path.dirname(path.abspath(__file__))) + '/config/gateway.yaml'.replace('/', path.sep))
