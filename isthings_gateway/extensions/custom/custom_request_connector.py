...
class CustomRequestConnector(Connector, Thread):
    ...
    def __send_request(self, request, converter_queue, logger):
        ...
        try:
            ...
            if request["config"].get("allowRedirects") is not None:
                params["allow_redirects"] = request["config"]["allowRedirects"]
    ...
    def __process_data(self):
        try:
            if not self.__convert_queue.empty():
                url, converter, config, data = self.__convert_queue.get()
                data_to_send = {}
                ...
                # else:
                data_to_send = converter.convert((url, config), data)
                if isinstance(data_to_send,list):
                    for device in data_to_send:
                        self.__gateway.send_to_storage(self.get_name(), device)
                        self.statistics["MessagesSent"] = self.statistics["MessagesSent"] + 1
                elif data_to_send:
                    self.__gateway.send_to_storage(self.get_name(), data_to_send)
                    self.statistics["MessagesSent"] = self.statistics["MessagesSent"] + 1
                # log.info("result=" + repr(data_to_send))
            else:
                sleep(.01)
        except Exception as e:
            log.exception(e)
